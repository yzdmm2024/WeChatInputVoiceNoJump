//
//  Tweak.m — 微信键盘免跳转 + 键盘外观定制 (rootless deb / ElleKit TweakInject)
//
//  目标进程（见 Filter.plist）：
...
// 我们的设置域（PreferenceLoader 写入此 suite）
...
// 设置键
static NSString *const kNoJumpEnabled  = @"wxkbdNoJumpEnabled";   // bool
static NSString *const kStyleEnabled   = @"wxkbdStyleEnabled";    // bool
static NSString *const kCornerRadius   = @"wxkbdCornerRadius";    // float 0-40
...
static NSString *const kBgAlpha        = @"wxkbdBgAlpha";         // float 0.2-1
static NSString *const kScale          = @"wxkbdScale";           // float 0.6-1.4
...
- (void)wx_handleItemClickEvent:(id)event func:(int)func controlEvent:(UIControlEvents)ctrl {
    if (func == 0x1 && wx_bool(kNoJumpEnabled, YES) && NSClassFromString(@"WBFunctionToolBar")) {
        ...
        // 找不到内建视图：交给 App 原生处理（此时 NSUserDefaults 键已被强制为 YES，原生逻辑不跳）
...
static void wx_entry(void) {
    @autoreleasepool {
        ...
        // 2) 语音按钮拦截（仅当 WBFunctionToolBar 存在时）
        ...
        // 5) 外观 hook（仅键盘扩展内有 WBInputViewController）
